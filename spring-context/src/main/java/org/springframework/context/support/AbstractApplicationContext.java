...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		//加锁是为了防止用时有多个线程刷新上下文，startupShutdownMonitor只是一个标记资源
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				/**
				 * 注册bean后置处理器，只是注册功能，真正调用的是getBean()方法
				 */
	 ...
	 *
	 * 配置当前工厂的标准上下文特征，例如上下文的类加载器和后置处理器
	 */
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		// Tell the internal bean factory to use the context's class loader etc.
		/**
		 * 设置当前工厂的类加载器，容器中使用的类加载器是同一个
		 */
		 ...
		 * 注意,这里只是添加到后置处理列表,并没有将其解析为beanDefinition,更没有解析为bean注入到容器当中.
	 ...
	 * 实例化并调用所有已注册的bean工厂后置处理器，严格执行其中的方法
	 * 必须在单例bean实例化之前调用
	 */
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		/**
		 * 首先获取容器中的BeanFactoryPostProcessors（
		 * BeanFactoryPostProcessors是bean工厂的属性，而beanPostProcessors是容器的属性）
		 * getBeanFactoryPostProcessors（）方法是获取手动注册的beanFactory的后置处理器
		 */
	 ...
	 *
	 * 完成容器bean工厂的初始化,初始化所有剩余的非懒加载的单例bean
	 */
